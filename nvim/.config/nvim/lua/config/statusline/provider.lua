...
function _G.ml_mode()
    ...
    local m = mode_alias[mode] or mode_alias[string.sub(mode, 1, 1)] or 'Unkown'
...
function M.gitinfo()
    ...
    for i = 2, 4 do
        ...
        api.nvim_set_hl(0, 'Statusline', { fg = color.bg })
    end
    return {
        stl = function()
...
function M.diagnostic()
    for i = 1, 3 do
        ...
        api.nvim_set_hl(0, 'Statusline', { fg = fg })
    end
    return {
        stl = diagnostic_info(),
...
function M.encoding()
    ...
    return {
        ...
        stl = (' %s'):format(map[vim.bo.fileencoding] or 'U'),
        name = 'filencode',
        event = { 'BufEnter' },
    }
end
